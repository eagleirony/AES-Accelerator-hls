...
int main(int argc, char** argv) {
    ...
    parser.addSwitch("--aes_per_kernel", "-apk", "AES runs per kernel", "max");
    ...
    std::string apk = parser.value("aes_per_kernel");
    ...
    for (int test = 0; test < num_tests; test++) {
        ...
        auto buf_out = xrt::bo(device, sizeof(uint8_t) * output_size, krnl.group_id(2));
        auto buf_key = xrt::bo(device, sizeof(uint8_t) * aes_key_size, krnl.group_id(3));
        ...
        while (true) {
            ...
            r = fread(buf_in_map, sizeof(uint8_t), input_size, input_fd);
            if (r == 0) {
                break;
            }
